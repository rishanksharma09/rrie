...
import { LayoutDashboard, Users, Bed, Bell, Activity, AlertCircle, X, Wifi, LogOut, Settings, BarChart3, ClipboardList, Shield, Clipboard, MapPin, Loader2, FileText, Printer } from 'lucide-react';
...
const HospitalPortal = () => {
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] flex font-sans overflow-x-hidden">
                                            ...
                                            <div className="flex flex-col md:flex-row items-start justify-between gap-6">
                                                ...
                                                <div className="flex flex-row md:flex-col items-center md:items-end gap-3 w-full md:w-auto mt-4 md:mt-0 pt-6 md:pt-0 border-t md:border-t-0 border-slate-100">
                                                    ...
                                                    <button
                                                        ...
                                                        className="w-12 h-12 rounded-2xl bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white border border-blue-100 hover:border-blue-600 transition-all flex items-center justify-center shrink-0"
            ...
            {/* Medical Handover Modal */}
            {selectedAlertForReport && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-10 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <div>
                                ...
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Generated by RRIE Intelligence Engine</p>
                            </div>
                            <button
                        ...
                        <div id="printable-handover-report" className="p-8 md:p-12 space-y-10 overflow-y-auto custom-scrollbar flex-1">
                            {/* Patient Info Header */}
                            <div className="grid grid-cols-2 gap-8 pb-8 border-b border-slate-100">
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Patient Subject</label>
                                    <div className="text-lg font-black text-slate-900 uppercase">{selectedAlertForReport.patientName || 'Anonymous Patient'}</div>
                                </div>
                                <div className="text-right">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Incoming Via</label>
                                    <div className="text-lg font-black text-blue-600 uppercase">{selectedAlertForReport.ambulanceUnit || 'Ambulance Dispatch'}</div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                <div className="flex items-center gap-4">
                                    ...
                                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">Clinical Triage Data</span>
                                    <div className="h-px flex-1 bg-slate-100" />
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-3">Classification</label>
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 bg-slate-900 text-white rounded-xl flex items-center justify-center font-black text-lg shadow-lg">
                                                {selectedAlertForReport.emergency_type?.[0]?.toUpperCase()}
                                            </div>
                                            <div className="font-black text-slate-900 uppercase tracking-tight">{selectedAlertForReport.emergency_type}</div>
                                        </div>
                                    </div>

                                    <div className={`p-6 rounded-[2rem] border-2 flex flex-col justify-center ${selectedAlertForReport.severity?.toLowerCase() === 'high' || selectedAlertForReport.severity?.toLowerCase() === 'critical'
                                        ? 'bg-red-50 border-red-100 text-red-600'
                                        : 'bg-orange-50 border-orange-100 text-orange-600'
                                        }`}>
                                        <label className="text-[10px] font-black opacity-50 uppercase tracking-widest block mb-1">Severity Tier</label>
                                        <div className="text-xl font-black uppercase tracking-widest">{selectedAlertForReport.severity}</div>
                                    </div>
                                </div>

                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                                        <Activity size={12} className="text-blue-600" /> AI Perspective & Reasoning
                                    </label>
                                    <div className="p-6 md:p-8 bg-blue-50/30 rounded-[2rem] border border-blue-100/50 italic text-slate-700 font-medium leading-[1.6]">
                                        "{selectedAlertForReport.reasoning}"
                                    </div>
                                </div>

                                {selectedAlertForReport.risk_flags?.length > 0 && (
                                    <div className="space-y-4">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Identified System Risks</label>
                                        <div className="flex flex-wrap gap-2">
                                            {selectedAlertForReport.risk_flags.map((flag: string) => (
                                                <div key={flag} className="flex items-center gap-2 bg-slate-900 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-tighter shadow-md">
                            ...
                            {/* Footer / Auth */}
                            <div className="pt-8 border-t border-slate-100 flex justify-between items-end opacity-40">
                                <div>
                                    <div className="text-[8px] font-black uppercase tracking-widest">Digital Auth Code</div>
                                    <div className="text-[10px] font-mono">RX-{hospitalId?.slice(-6).toUpperCase()}-{Date.now().toString().slice(-4)}</div>
                                </div>
                                <div className="text-right">
                                    <div className="text-[8px] font-black uppercase tracking-widest">Generated On</div>
                                    <div className="text-[10px] font-black">{new Date().toLocaleString()}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-8 border-t border-slate-100 bg-white flex gap-4">
                            <button
                                ...
                                className="flex-1 py-5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-[0.3em] flex items-center justify-center gap-3 hover:bg-slate-800 transition-all shadow-xl shadow-slate-900/20"
                            >
                                <Printer size={18} />
            ...
            <style dangerouslySetInnerHTML={{
                ...
                @media print {
                    ...
                    /* ONLY show the handover report and its children */
                    #printable-handover-report, #printable-handover-report * {
                        visibility: visible;
                    }
                    #printable-handover-report {
                        position: absolute;
                        left: 0;
                        top: 0;
                        width: 100%;
