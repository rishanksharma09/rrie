...
const UserPortal = () => {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            // If an ambulance was dispatched, switch to bookings tab to see live tracking
    ...
    useEffect(() => {
        if (activeTab === 'bookings' && bookings.length > 0 && mapContainerRef.current) {
            ...
            if (!mapRef.current) {
                ...
                // Create Patient Marker (Red Emergency Dot)
                ...
                // Create Ambulance Marker (Truck Icon)
        ...
        return (
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
                <div className="max-w-md w-full bg-white rounded-3xl shadow-xl shadow-blue-500/10 p-8 border border-slate-100 text-center">
                    ...
                    <h1 className="text-3xl font-black text-slate-900 mb-2">User Portal</h1>
                    <p className="text-slate-500 mb-8">Please sign in to request emergency referrals and track ambulances.</p>
                    <button
                        onClick={login}
            ...
            <aside className="w-20 md:w-64 bg-white border-r border-slate-200 flex flex-col items-center md:items-stretch py-8 px-4 fixed h-full z-20">
                ...
                <div className="space-y-2 flex-1">
                    ...
                    >
                        ...
                        <span className="hidden md:block font-bold">Analyze Symptoms</span>
                    </button>
                    ...
                    >
                        <MapIcon size={22} />
                        <span className="hidden md:block font-bold">Ambulance Tracking</span>
                    </button>
                </div>
                ...
                {activeTab === 'analyze' ? (
                    <div className="max-w-3xl mx-auto space-y-8">
                        <div className="flex items-center justify-between">
                            <div>
                                <h2 className="text-3xl font-black text-slate-900">Emergency Triage</h2>
                                <p className="text-slate-500 font-medium">Describe your emergency and we'll handle the rest.</p>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
                            <div className="p-8 md:p-10 space-y-8">
                                <div>
                                    <label className="block text-sm font-black text-slate-400 uppercase tracking-widest mb-4">What's happening?</label>
                                    <div className="relative group">
                                        <textarea
                                            ...
                                            placeholder="Tell us everything you feel..."
                                            value={symptoms}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex items-center gap-3 bg-red-50 p-5 rounded-2xl border border-red-100 transition-all hover:bg-red-100/50 cursor-pointer" onClick={() => setWantAmbulance(!wantAmbulance)}>
                                            ...
                                            />
                                            <span className="text-sm font-black text-red-900 leading-tight">I need immediate ambulance dispatch</span>
                                        </div>
                                    </div>
                                </div>
                        ...
                        {/* Result Section (Same as before but nested) */}
                        {result && (
                                ...
                                <div className="bg-white rounded-[2.5rem] shadow-2xl border border-blue-100/50 overflow-hidden">
                                    <div className="bg-green-50/50 px-10 py-6 border-b border-green-100/50 flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h3 className="font-black text-green-900 text-xl tracking-tight">Referral Active</h3>
                                                <p className="text-xs text-green-600 font-bold uppercase tracking-widest">Confidence Score: {result.confidence}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-10 space-y-10">
                                        <section>
                                            <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-4">Diagnosis Reasoning</div>
                                        ...
                                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                            <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                                                <div className="text-[10px] font-black text-blue-600 uppercase tracking-[0.3em] mb-4">Assigned Hospital</div>
                                            ...
                                            <div className={`p-8 rounded-[2rem] border-2 flex flex-col justify-center ${result.ambulance.includes('not requested') ? 'border-dashed border-slate-200 bg-white' : 'border-red-100 bg-red-50/30'}`}>
                                                <div className="text-[10px] font-black text-red-500 uppercase tracking-[0.3em] mb-4">Transport Status</div>
                                                <h4 className="text-2xl font-black text-slate-900 mb-2">{result.ambulance}</h4>
                                                <div className="text-lg font-bold text-red-600">ETA: {result.eta}</div>
                                            </div>
                                        </div>
                                        ...
                                        {result.alternatives && result.alternatives.length > 0 && (
                                            <section className="pt-10 border-t border-slate-100">
                                                <div className="flex items-center gap-2 mb-8">
                                                    <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Secondary Facilities</h4>
                                                        ...
                                                        <div key={idx} className="group relative bg-white p-6 hover:bg-slate-50 rounded-3xl transition-all border border-slate-100 hover:border-blue-100 hover:shadow-xl hover:shadow-blue-500/5">
                                                            <div className="flex items-start justify-between gap-4">
                                                                ...
                                                                <div className="flex flex-col items-end gap-2">
                                                                    <div className="text-[10px] font-black text-blue-600 tracking-tighter bg-blue-50 px-2 py-1 rounded-lg border border-blue-100">MATCH: {alt.score}</div>
                    ...
                    <div className="max-w-4xl mx-auto space-y-8">
                        <div>
                            <h2 className="text-3xl font-black text-slate-900">Live Ambulance Tracking</h2>
                            <p className="text-slate-500 font-medium">Monitor your active medical dispatch in real-time.</p>
                        </div>
                            ...
                            <div className="lg:col-span-1 space-y-4 overflow-y-auto pr-2 custom-scrollbar">
                                {isLoadingBookings ? (
                                    ...
                                    <div className="bg-white p-10 rounded-3xl border border-dashed border-slate-200 text-center">
                                        ...
                                        <p className="text-slate-400 font-bold">No active bookings found.</p>
                                    </div>
                                ) : (
                                    bookings.map((booking) => (
                                        <div key={booking._id} className="bg-white p-6 rounded-3xl border-2 border-transparent hover:border-blue-500/30 transition-all cursor-pointer shadow-lg shadow-slate-200/50 group">
                                            ...
                                            <h4 className="font-black text-slate-900 mb-1 group-hover:text-blue-600 transition-colors uppercase text-sm tracking-tight">{booking.assignedHospital?.name || 'Emergency Center'}</h4>
                                            <p className="text-xs text-slate-500 font-bold mb-4">{booking.assignedAmbulance?.vehicleNumber || 'Pending Dispatch'}</p>
                                            <div className="flex gap-2">
                                                <div className="bg-blue-50 text-blue-600 text-[10px] font-black px-3 py-1.5 rounded-lg border border-blue-100 flex-1 text-center">ETA: {booking.assignedAmbulance?.eta || 'CALCULATING'}</div>
                                            </div>
                                        </div>
                                    ))
                                )}
                            ...
                            <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl relative overflow-hidden">
                                {bookings.length > 0 ? (
                                        ...
                                        {bookings[0]?.assignedAmbulance?.id?.contactNumber && (
                                            <div className="absolute bottom-6 left-6 right-6 bg-white/90 backdrop-blur-md p-6 rounded-3xl shadow-2xl border border-white/50 flex items-center justify-between z-10">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Ambulance Driver</div>
                                                        <div className="font-black text-slate-900">{bookings[0].assignedAmbulance.id.driverName || 'Dispatch Officer'}</div>
                                                    </div>
                                                </div>
                                    ...
                                    <div className="w-full h-full bg-slate-50 flex items-center justify-center">
                                        <p className="text-slate-400 font-medium">Select an active booking to track on map.</p>
                                    </div>
                                )}
                            </div>
                        </div>
